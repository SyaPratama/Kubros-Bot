...
import { DB as db } from "./database.js";
...
export const serialize = (sock, m) => {
  ...
  if (m.message) {
    ...
    if (m.quoted) {
      m.quoted = extractQuotedMessage(sock, m);
    }
  }
  ...
  m.senderIsOwner = Object.keys(m.db.bot.owners).includes(
    m.sender.split("@")[0]
  )
    ? true
    : false;
...
const messageWrapper = (sock, m) => {
  m._reply = async (text) => {
    ...
    await delay(500);
    await sock.sendPresenceUpdate("pause", m.chat);
