...
import { commands } from "../../helper/command.js";
import { DATABASE_SCHEMA } from "../../config.js";
import { DB as db } from "../../helper/database.js";
import { moment } from "../../helper/moment.js";
import { middleware } from "../middleware/app.js";
import { serialize, updateAdminStatus } from "../../helper/serialize.js";
  ...
  sock.ev.on("messages.upsert", async (update) => {
    try {
      ...
      const command = Array.from(commands.values()).find((v) =>
        v.cmd.find(
          (x) => x.toLowerCase() == m.body.commandWithoutPrefix.toLowerCase()
        )
      );
  ...
  for (let [key, middleware] of middlewares.entries()) {
    try {
      if (!command?.withoutMiddleware) {
        await middleware.default.handler(sock, m, true, command);
